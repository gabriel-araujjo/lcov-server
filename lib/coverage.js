...
const uReport = (pid, bra) => ({
  name: 'update_report',
  text: 'UPDATE report ' +
      'SET bra = null ' +
      'WHERE pid = $1 AND bra = $2',
  values: [pid, bra]
})
...
const fFuncCovByFile = (rid, fid) => ({
  name: 'find_func_cov_by_rep',
  text: 'SELECT sln, eln, fna, hit, bct, bex FROM func_cov ' +
      'WHERE rid = $1 AND fid = $2',
  values: [rid, fid],
  rowMode: 'array'
})
...
module.exports.save = async function(message) {
  ...
  try {
    ...
    if (bra) {
      // First alter any existent report that points to a symbolic name.
      // A symbolic name is the branch name, such as: master, dev; or a tag
      // name, such as: v1.0.0, v2.3.0, etc.
      await db.query(uReport(pid, bra))
    }
