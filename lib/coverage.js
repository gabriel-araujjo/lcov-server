...
function calcCoveragePercentage(coverage) {
  let blockCount = 0
  let blocksHit = 0
  for (const [, , func] of coverage) {
    for (const [, , , count, hits] of func) {
      blockCount += count
      blocksHit += hits
    }
  }
...
const fReport = (pid, com) => ({
  name: 'find_reports',
  text: 'SELECT gid, bra, rat, ci, job, prq, bct, bex FROM report ' +
...
module.exports.save = async function(message) {
  ...
  try {
    ...
    const [rid] =
        await q(cReport(pid, com, bra, rat, ci, job, prq, ...covCache))
