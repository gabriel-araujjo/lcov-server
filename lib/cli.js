...
module.exports = function cli({parser, input, url, basePath}) {
  ...
  return new Promise(async function(resolve, reject) {
    try {
      ...
      output['source_files'] = await parsers[parser].parse(input)

      // Go through and set the file contents
      for (let i = 0; i < output['source_files'].length; i++) {
        let path = basePath ?
            Path.resolve(
                process.cwd(), basePath, output['source_files'][i].file) :
            output['source_files'][i].file;

        if (fs.existsSync(path)) {
          output['source_files'][i].source =
              fs.readFileSync(path).toString('utf8');
          output['source_files'][i].title =
              output['source_files'][i].file.substring(
                  output['source_files'][i].file.lastIndexOf('/') + 1,
                  output['source_files'][i].file.length);
          output['source_files'][i].file =
              output['source_files'][i].file.replace(basePath, '');
          if (output['source_files'][i].file[0] == '/') {
            output['source_files'][i].file =
                output['source_files'][i].file.substring(1);
          }
        } else {
