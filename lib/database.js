...
const createMigrationTable = {
  text: 'CREATE TABLE migration_ctrl ' +
...
async function getDatabaseVersion(shouldThrow=false) {
  ...
  try {
    ...
    client.query(createMigrationTable);
    return await getDatabaseVersion(true);
  }
}
...
syncDb().catch(err => {
  console.error('Cannot setup database', err);
  process.exit(1);
})
